...
def print_embeddings( tokenizer, vocab_embeddings, your_embeddings, embeddings_to_compare ):
    ...
    for idx_word_embedding in range(your_embeddings.shape[0]):
        word_embedding = your_embeddings[idx_word_embedding]
        other_embedding = vocab_embeddings[embeddings_to_compare[idx_word_embedding]]
        diffs = torch.abs(word_embedding-other_embedding)
        sorted_values, indexes = torch.sort(diffs, descending=True)
        look_for = 20
        print(f"Top {look_for} positions:")
        for i in range(look_for):
            print( f'Posição: {indexes[i].item()} - Diferença: {sorted_values[i].item()}', flush=True )


        selected_indexes = indexes[:look_for]
        distances = torch.norm(vocab_embeddings[:,selected_indexes] - word_embedding[selected_indexes], dim=1)
        sorted_ditances, indexes_distances = torch.sort(distances, descending=False)

        print('For {idx_word_embedding}º token:', flush=True )
        for i in range(10):
            print( f'N. token: {indexes_distances[i].item()} - Distância: {sorted_ditances[i].item()} - token: {tokenizer.decode([indexes_distances[i].item()], skip_special_tokens=False)}', flush=True )
...
def main( local_rank, global_rank, world_size, random_seed, model_file, final_domain ):
    ...
    print_embeddings( tokenizer, phrase_gen.weights_values, phrase_gen.learnable_domain_weights, tokenizer.encode(final_domain) )

    # output_ids = phrase_gen( {'input_ids':torch.tensor(tokenizer.encode('Computer chairs on sale. Enjoy comfort while gaming!')).reshape(1,-1).to(local_rank)} )
...
if __name__ == "__main__":
    ...
    model_file = f"final_models/convert_laptop_to_reddit_games__intermediate_sport_in_german__final_games_in_portuguese__seed_13.pt"
    # model_file = f"final_models/convert_laptop_to_reddit_games__intermediate_sport_in_german__final_movie__seed_13.pt"
